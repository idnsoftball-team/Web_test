  ...
  if(homeLeaveDiv) {
      ...
      if(todayLeaves.length === 0) {
          ...
          todayLeaves.forEach(l => {
              homeLeaveDiv.innerHTML += `<div style="background:white; padding:10px; border-radius:8px; margin-bottom:5px; border-left:3px solid #ffcc00; font-size:0.9rem; display:flex; justify-content:space-between;"><strong>${escapeHtml(l.name)}</strong> <span style="color:#666">${l.slot}</span></div>`;
          });
      }
  }
  renderLeaveList();
...
function renderLeaveList() {
    ...
    list.forEach(l => {
        d.innerHTML += `<div class="leave-card-new">
            ...
            <div class="leave-meta-row"><div class="meta-tag"><i class="far fa-calendar-alt"></i> ${l.date}</div><div class="meta-tag"><i class="far fa-clock"></i> ${escapeHtml(l.slot)}</div></div>
...
function renderMatches() {
    ...
    const list = matches.filter(m => { if(m.type==='singles'&&!s)return false; if(m.type==='doubles'&&!db)return false; const t=[...m.players,...m.opponents].map(getPlayerName).join(' ').toLowerCase(); return !k||t.includes(k); });
    ...
    list.forEach(m => {
        ...
        <div class="match-details-panel">
            <div style="margin-bottom:5px;"><strong>局數:</strong> ${escapeHtml(m.sets||'無')}</div>
            ${m.video.url?`<button class="hero-btn" style="font-size:0.8rem;padding:4px 10px;" onclick="event.stopPropagation();window.open('${m.video.url}','_blank')">影片</button>`:''}
        </div>`;
            ...
            Object.keys(slots).forEach(s=>{
                ...
                items.forEach(e=>{
                    let html=`<div class="compact-card">
                        ...
                        <div class="player-name">${escapeHtml(e.playerA?.name||'')}</div>
                        ${e.playerB&&e.playerB.name?`<div class="player-name">${escapeHtml(e.playerB.name)}</div>`:''}
                    </div>`;
                    g.innerHTML+=html;
                });
                ct.appendChild(g)
