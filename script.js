// script.js - v25.0 (Admin Leave: Edit, Filter, Sort)
...
// ... (Clear SW & Init Logic - Same as before) ...
...
let adminLeaveShowToday = false; // State for admin filter
...
// ... (normalizeData, renderHome... renderMedia functions 保持不變，為節省篇幅省略，請直接使用 v24.0 的這部分) ...
// 請務必保留 normalizeData 到 renderMedia 的所有程式碼！
// 若您需要我再貼一次完整版請告知，這裡我只貼修改的 Admin 部分
...
function renderLeaveList() {
    ...
    let c = document.querySelector('.leave-controls');
    if (!c && d.parentElement.querySelector('h2')) {
        c = document.createElement('div'); c.className = 'leave-controls';
        c.innerHTML = `<button id="btn-filter-today" class="btn-filter-today"><i class="fas fa-calendar-day"></i> 只看今日</button>`;
        d.parentElement.insertBefore(c, d);
        document.getElementById('btn-filter-today').onclick = () => { adminLeaveShowToday=!adminLeaveShowToday; document.getElementById('btn-filter-today').classList.toggle('active',adminLeaveShowToday); renderLeaveList(); };
    }
    let l = leaveRequestsData.slice().sort((a, b) => new Date(a.date) - new Date(b.date)); // Sort asc
    if (adminLeaveShowToday) { const t=new Date().toISOString().split('T')[0]; l=l.filter(x=>x.date===t); }
    if(l.length===0) { d.innerHTML='<div style="text-align:center;color:#888;padding:20px">無資料</div>'; return; }
    const todayStr = new Date().toISOString().split('T')[0];
    l.forEach(i => {
        const isT=i.date===todayStr;
        d.innerHTML += `<div class="leave-card-new ${isT?'is-today':''}"><div class="leave-header-row"><span class="leave-name-large">${escapeHtml(i.name)}</span>${isT?'<span style="color:#e67e22;font-weight:bold;font-size:0.8rem;">TODAY</span>':''}</div><div class="leave-meta-row"><div class="meta-tag"><i class="far fa-calendar-alt"></i> ${i.date}</div><div class="meta-tag"><i class="far fa-clock"></i> ${escapeHtml(i.slot)}</div></div><div class="leave-reason-box">${escapeHtml(i.reason)}</div></div>`;
    });
}
...
function showAdminLeaveList() {
    ...
    let html = `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        <h3>請假管理</h3>
        <button id="adm-filter-today" class="btn-filter-today ${adminLeaveShowToday?'active':''}"><i class="fas fa-calendar-day"></i> 只看今日</button>
    </div>`;
    
    // Sort & Filter
    let list = leaveRequestsData.slice().sort((a,b) => new Date(a.date) - new Date(b.date)); // Nearest first
    if(adminLeaveShowToday) {
        const today = new Date().toISOString().split('T')[0];
        list = list.filter(l => l.date === today);
    }

    if(list.length === 0) html += '<div class="card">無資料</div>';
    else {
        list.forEach(l => {
            html += `
    ...
    // Re-bind filter button
    document.getElementById('adm-filter-today').onclick = () => {
        adminLeaveShowToday = !adminLeaveShowToday;
        showAdminLeaveList();
    };
}
