// script.js - v9.0 Logic Fixes (Announcement Data & New Visual Renders)
...
// === 1. Data Loading & Robust Normalization ===
...
function normalizeData(data) {
  ...
  // Announcements (Fixing "Can't read data")
  const anns = (data.announcements || []).map(r => ({
    title: getVal(r, ['title', '標題', 'announcement_title', 'Title']),
    date: getVal(r, ['date', '日期', 'announcement_date', 'Date']),
    content: getVal(r, ['content', '內容', 'announcement_content', 'Content'])
  })).filter(a => a.title); // Filter out empty ones
  ...
  const mapPlayers = (data.players || []).map(r => ({
    ...
    paddle: getVal(r, ['paddle', 'team_no', '膠皮'])
  }));
  ...
  const leaves = (data.leave_requests || []).map(r => ({
    rowId: r.rowId,
    name: getVal(r, ['created_by_email', 'name', '姓名']),
    date: getVal(r, ['leave_date', 'date', '日期']),           
  ...
  const mapMatches = (data.matches || []).map(r => ({
    rowId: r.rowId,
    date: getVal(r, ['match_date', 'date', '日期']),
...
// NEW: Leave List (Date/Time separated & Colored)
function renderLeaveList() {
    ...
    leaveRequestsData.forEach(item => {
        // Simple formatting for date if possible (remove time part if strictly YYYY-MM-DD is needed)
        ...
        card.innerHTML = `
            <div class="leave-header-row">
                <span class="leave-name-large">${escapeHtml(item.name)}</span>
            </div>
...
// NEW: Match Records (Scoreboard Style)
      ...
      <div class="match-score-body">
        <div class="match-players-container">
           <div class="match-side">
              <span class="side-names">${escapeHtml(pNames)}</span>
           </div>
           <div style="font-size:0.8rem;color:#ccc;">VS</div>
           <div class="match-side">
              <span class="side-names">${escapeHtml(oNames)}</span>
           </div>
        </div>
...
// Render Announcements (Fixing Issue #4)
function renderHome() {
  ...
  // Render today leaves snippet (simplified)
  renderLeaveList(); // Re-use the main function but we might want to just show top 3 in home? 
  // For now, let's just make sure the main list works. The home snippet code is in previous versions if needed.
}
...
async function sendToGas(action, payload) { /* Auth logic preserved */ const res=await fetch(GAS_API_URL,{method:'POST',body:JSON.stringify({action,payload})}); const j=await res.json(); showToast(j.message); if(j.success) loadAllData(); }
function renderAdmin() { /* Auth check logic preserved */ if(!sessionStorage.getItem('admin_pwd')) { document.getElementById('admin-login').classList.remove('hidden'); document.getElementById('admin-dashboard').classList.add('hidden'); document.getElementById('admin-login-btn').onclick=async()=>{ /* login */ }; } else { document.getElementById('admin-login').classList.add('hidden'); document.getElementById('admin-dashboard').classList.remove('hidden'); } }
